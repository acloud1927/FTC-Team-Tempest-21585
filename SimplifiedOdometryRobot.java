...
public class SimplifiedOdometryRobot{
    ...
    private final double ODOM_INCHES_PER_COUNT   = 0.001935;   //  GoBilda Odometry Pod (1/226.8)
    ...
    private DcMotor Front_left;     //  control the left front drive wheel
    private DcMotor Front_right;    //  control the right front drive wheel
    private DcMotor Back_left;      //  control the left back drive wheel
    private DcMotor Back_right;     //  control the right back drive wheel

    private DcMotor axial;       //  the Axial (front/back) Odometry Module (may overlap with motor, or may not)
    private DcMotor lateral;      //  the Lateral (left/right) Odometry Module (may overlap with motor, or may not)
    ...
    public void initialize(boolean showTelemetry)
    {
        ...
        Front_left  = setupDriveMotor("Front_left", DcMotor.Direction.REVERSE);
        Front_right = setupDriveMotor("Front_right", DcMotor.Direction.FORWARD);
        Back_left  = setupDriveMotor( "Back_left", DcMotor.Direction.REVERSE);
        Back_right = setupDriveMotor( "Back_right",DcMotor.Direction.FORWARD);
        ...
        axial = myOpMode.hardwareMap.get(DcMotor.class, "leftLift");
        lateral = myOpMode.hardwareMap.get(DcMotor.class, "rightLift");
    ...
    public boolean readSensors() {
        rawDriveOdometer = axial.getCurrentPosition() * (INVERT_DRIVE_ODOMETRY ? -1 : 1);
        rawStrafeOdometer = lateral.getCurrentPosition() * (INVERT_STRAFE_ODOMETRY ? -1 : 1);
    ...
    public void drive(double distanceInches, double power, double holdTime) {
        ...
        while (myOpMode.opModeIsActive() && readSensors()){
            ...
            moveRobotForward(driveController.getOutput(driveDistance), yawController.getOutput(heading));
    ...
    public void strafe(double distanceInches, double power, double holdTime) {
        ...
        while (myOpMode.opModeIsActive() && readSensors()){
            ...
            // implement desired axis powers
            moveRobot(driveController.getOutput(driveDistance), strafeController.getOutput(-strafeDistance), yawController.getOutput(heading));
            //moveRobotStrafe(strafeController.getOutput(strafeDistance), yawController.getOutput(heading));
     ...
     * @param holdTime Minimum time (sec) required to hold the final position.  0 = no hold.
     */
    public void turn(double headingDeg, double power, double holdTime) {
    ...
    public void moveRobot(double drive, double strafe, double yaw){
        ...
        Front_left.setPower(lF);
        Front_right.setPower(rF);
        Back_left.setPower(lB);
        Back_right.setPower(rB);
    ...
    public void moveRobotForward(double drive, double yaw){
        ...
        if (showTelemetry) {
            // myOpMode.telemetry.addData("Axes D:S:Y", "%5.2f %5.2f %5.2f", drive, yaw);
            //  myOpMode.telemetry.addData("Wheels lf:rf:lb:rb", "%5.2f %5.2f %5.2f %5.2f", lF, rF, lB, rB);
            myOpMode.telemetry.update(); //  Assume this is the last thing done in the loop.
        }
    }
    public void moveRobotStrafe(double strafe, double yaw){

        double lF = strafe - yaw;
        double rF = strafe + yaw;
        double lB = strafe - yaw;
        double rB = strafe + yaw;
